...
def build_fixed_router(repository: Callable[[], HerCareRepository]) -> APIRouter:
    ...
    def create_check_in(payload: CheckInRequest, response: Response, idempotency_key: str | None = Header(default=None, min_length=1, max_length=128), repo: HerCareRepository = Depends(repository)) -> dict:
        data = payload.model_dump()
        data["metric_type"] = data.get("metric_type") or "daily_check_in"
        data["value"] = data.get("value") or data.get("symptoms") or data.get("mood") or "recorded"
        result, created = repo.create_check_in(idempotency_key=idempotency_key or f"checkin-{uuid4().hex}", data=data)
    ...
    def update_care_plan(item_id: str, payload: CarePlanUpdate, repo: HerCareRepository = Depends(repository)) -> dict:
        try:
            return repo.update_care_plan(item_id, payload.resolved_completed)
